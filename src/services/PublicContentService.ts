...
export class PublicContentService {
  ...
  async getContentByType(type: 'article' | 'news' | 'announcement' | 'event', filters?: Omit<ContentFilter, 'type'>): Promise<ContentListResponse> {
    return contentRepository.getAllContent({ ...filters, type })
  }
  ...
  async getNews(filters?: Omit<ContentFilter, 'type'>): Promise<ContentListResponse> {
    return this.getContentByType('news', filters)
  }

  async getArticles(filters?: Omit<ContentFilter, 'type'>): Promise<ContentListResponse> {
    return this.getContentByType('article', filters)
  }

  async getAnnouncements(filters?: Omit<ContentFilter, 'type'>): Promise<ContentListResponse> {
    return this.getContentByType('announcement', filters)
  }

  async getEvents(filters?: Omit<ContentFilter, 'type'>): Promise<ContentListResponse> {
    return this.getContentByType('event', filters)
  }
  ...
  async getHomepageContent(): Promise<{
    ...
    const [featured, latest, newsResponse, announcementsResponse, faqs] = await Promise.all([
      ...
      this.getNews({ page: 1, pageSize: 4 }),
      this.getAnnouncements({ page: 1, pageSize: 4 }),
      this.getAllFAQs()
    ])
