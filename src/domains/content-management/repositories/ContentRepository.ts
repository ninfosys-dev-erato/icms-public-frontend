...
export class ContentRepository {
  ...
  async getCategoryTree(): Promise<Category[]> {
    try {
      const response = await publicApiClient.get('/categories/tree')
      if (response.success) {
        return response.data as Category[]
      }
      throw new Error(response.error || 'Failed to fetch category tree')
    } catch (error) {
      console.warn('Failed to fetch category tree:', error)
      return []
    }
  }
  ...
  async getActiveCategories(): Promise<Category[]> {
    try {
      ...
      throw new Error(response.error || 'Failed to fetch active categories')
    } catch (error) {
      console.warn('Failed to fetch active categories:', error)
      return []
    }
  }
  ...
  async getCategoryBySlug(slug: string): Promise<Category | null> {
    try {
      const response = await publicApiClient.get(`/categories/${slug}`)
      if (response.success) {
        return response.data as Category
      }
      return null
